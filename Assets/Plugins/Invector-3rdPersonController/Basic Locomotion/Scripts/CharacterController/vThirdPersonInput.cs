...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonInput : vMonoBehaviour, vIAnimatorMoveReceiver
    {
        ...
        public bool enableMobileControllers;
        ...
        public virtual void MoveInput()
        {
            if (!lockMoveInput)
            {
                ...
                input.x = ControlFreak2.CF2Input.GetAxis("Horizontal");
                input.z = ControlFreak2.CF2Input.GetAxis("Vertical");
#else
                var p = PlayerPrefs.GetString("Platform", "Windows");
                if (p=="Android")
                {
                    input.x = ControlFreak2.CF2Input.GetAxis("Horizontal");
                    input.z = ControlFreak2.CF2Input.GetAxis("Vertical");
        ...
        public virtual bool JumpConditions()
        {
            return !cc.inJumpStarted && !cc.customAction && !cc.isCrouching && cc.isGrounded &&
                   !((int)cc.GroundAngle() > cc.slopeLimit) && cc.currentStamina >= cc.jumpStamina && !cc.isJumping &&
                   !cc.isRolling;
        }
        ...
        public virtual void CameraInput()
        {
            ...
            float X = 0f;
#if UNITY_ANDROID || UNITY_IOS
             Y = lockCameraInput ? 0f : ControlFreak2.CF2Input.GetAxis("Vertical2");
             X = lockCameraInput ? 0f : ControlFreak2.CF2Input.GetAxis("Horizontal2");
#else
            var p = PlayerPrefs.GetString("Platform", "Windows");
            ...
            {
                Y = lockCameraInput ? 0f : rotateCameraYInput.GetAxis();
                X = lockCameraInput ? 0f : rotateCameraXInput.GetAxis();
            }
#endif
